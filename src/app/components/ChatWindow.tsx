...
import { UploadDocumentsForm } from "@/app/components/UploadDocumentsForm";
...
export function ChatWindow(props: {
  ...
  const { endpoint, emptyStateComponent, placeholder, titleText = "Prospective", showIngestForm, showIntermediateStepsToggle, emoji } = props;
  ...
  return (
    // Message box frame
      <div className={`flex flex-col mx-auto mt-4 bg-white items-center p-4 max-h-[90vh] md:h-auto md:mt-[100px] md:max-h-[90vh] w-[90%] md:w-[80%]
      md:p-4 md:my-4 rounded-lg grow overflow-scroll border border-blue-500 ${(messages.length > 0 ? "border" : "")}`}>
        <h2 className={`${messages.length > 0 ? "" : "hidden"} text-lg md:text-2xl`}>{emoji} {titleText}</h2>
        {messages.length === 0 ? emptyStateComponent : ""}
        <div
          className="flex flex-col-reverse flex-grow w-full mb-2 md:mb-4 overflow-auto"
        ...
        <form onSubmit={sendMessage} className="flex w-full flex-col items-center">
          <div className="flex">
            {intermediateStepsToggle}
          </div>
          <div className="flex w-[80%] mt-4">
            <input
              className="grow mr-2 md:mr-8 p-2 md:p-4 border rounded w-[90%] md:w-auto"
              value={input}
              placeholder={placeholder ?? "Please enter a message"}
              onChange={handleInputChange}
            />
            <button type="submit" className="shrink-0 px-4 md:px-8 py-2 md:py-4 bg-blue-500 text-white rounded w-20 md:w-28">
